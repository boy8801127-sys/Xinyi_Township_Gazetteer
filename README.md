...
- 多頁式網站架構：左側可伸縮導覽選單（`gr.Sidebar`）＋ `Blocks.route()` 建立的獨立頁面——首頁是問答系統，另外還有「資料來源」（即時統計向量庫實際索引的論文／書籍來源，不是靜態寫死的清單）、「技術說明」（RAG／LangChain chain vs agent／fine-tuning 各模組架構與實測數據摘要）、「更新日誌」（依開發歷程整理），以及一個連到語意空間 PCA 視覺化（另一個獨立 artifact）的外部連結
