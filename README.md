...
在 Notion 自動分類流程產出的標註資料（`results/*.json`）之上，額外做了一個**平行的實驗性模組**，把已分類段落建成可語意檢索的知識庫，不影響、也不依賴正式的 `notion_classify.py` 流程。這是作品集／技術展示用途，示範 RAG 架構的實作方式：Retrieval（LlamaIndex + Chroma 向量庫 + Voyage embeddings）＋ Generation（Claude 依檢索結果生成含引用的回答）。
...
- **查詢引擎**（`src/rag/query_engine.py`）：
  ...
  - `answer_question(question, k)`：檢索＋用 Claude 生成有依據的回答，並回傳引用的來源清單（論文＋頁碼），降低憑空捏造答案的風險
